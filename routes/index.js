...
const wsdlUrl =
  'http://tdl.integration.aws.fema.gov/IPAWS_CAPService/IPAWS?wsdl';
...
function saveXml(
  ...
  xw.startElement('alert').writeAttribute(
    'xmlns',
  ...
  if (layersJson != null) {
    for (var i = 0; i < layersJson.length; i++) {
      ...
      if (shape == 'circle') {
        ...
        var radiusMeters = parseFloat(
          coordinates.substring(separateCircle + 1, coordinates.length - 1)
        );
    ...
    if (layersJson != null) {
      for (var i = 0; i < layersJson.length; i++) {
        ...
        if (shape == 'circle') {
          ...
          radiusKm = radiusKm.toFixed(1);
          coordinates =
            coordinates.substring(0, separateCircle) + ' ' + radiusKm;
        }
